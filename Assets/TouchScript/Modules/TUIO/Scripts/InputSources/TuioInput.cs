    ...
    public class TuioInput : InputSource
    {
      ...
      /*  private Text m_showText;
        ...
        void Start()
        {
            ...
            /*m_showText = GameObject.Find("Text").GetComponent<Text>();
            m_showText1 = GameObject.Find("Text1").GetComponent<Text>();*/
           // m_showText.text = m_showText1.text = "test";
        }
        ...
        //进行TUIO坐标还原
        void DecryptPoint(ref float x, ref float y)
        {
            float xTemp = 0, yTemp = 0;
            //先将坐标逆时针旋转78度
            float angle = -78f * 3.141592653f / 180f;
            xTemp = x * Mathf.Cos(angle) - y * Mathf.Sin(angle);
            yTemp = x * Mathf.Sin(angle) + y * Mathf.Cos(angle);
            //再进行缩放
            xTemp = xTemp / 0.86f;
            yTemp = yTemp / 0.79f;
            //再进行 x y互换
